	...
	<section class="section_gap_bottom">
		<div class="product_description_area">
			<div class="container">
				...
				<li class="nav-item">
					<a class="nav-link" id="review-tab" data-toggle="tab" href="#akun" role="tab" aria-controls="akun" aria-selected="false">Akun</a>
				</li>
				<li class="nav-item">
			...
			<div class="tab-content" id="myTabContent">
				<div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
					<div class="row">
						...
						$sql_profile = mysqli_query($koneksi, "SELECT * FROM pelanggan  WHERE id_akun_pelanggan='$_SESSION[id_akun_pelanggan]'");
						while($result_profile = mysqli_fetch_array($sql_profile)){
						?>
						<div class="col-lg-4">
							<img src="img/profile/<?= $result_profile['foto_pelanggan']; ?>" style="width: 300px; height: 300px; object-fit: cover;"> 
						</div>
						<div class="col-lg-8">
						<button class="btn btn-warning text-light" data-toggle="modal" data-target="#editProfileModal" style="float: right;">Edit</button>
							<h1 class="profile_name"><?= $result_profile['nama_lengkap']; ?></h1>
							<hr>
							<table style="font-size: 18px;">
								<tr style="height: 50px;">
									<td>Tempat, Tanggal Lahir</td>
									<td>:</td>
									<td style="color: #ff6c00;"><?= $result_profile['tempat_lahir']; ?>, <?= $result_profile['tanggal_lahir']; ?></td>
								</tr>
								<tr style="height: 50px;">
									<td>Jenis Kelamin</td>
									<td>:</td>
									<td style="color: #ff6c00;"><?= $result_profile['jenis_kelamin']; ?></td>
								</tr>
								<tr style="height: 50px;">
									<td>No. HP</td>
									<td>:</td>
									<td style="color: #ff6c00;"><?= $result_profile['no_hp']; ?></td>
								</tr>
							</table>
						</div>
						<?php } ?>
				...
				<div class="tab-pane fade" id="akun" role="tabpanel" aria-labelledby="akun-tab">
					<div class="row">
						<div class="col-lg-6">
							...
							<?php
							include "../../../koneksi.php";
							$sql_v_info_akun = mysqli_query($koneksi, "SELECT * FROM akun_pelanggan WHERE id_akun_pelanggan='$_SESSION[id_akun_pelanggan]'");
							while($result_v_info_akun = mysqli_fetch_array($sql_v_info_akun)){
							?>
							<div class="form-group">
								<label>Username</label>
								<input type="text" class="form-control" name="username" value="<?= $result_v_info_akun['username']; ?>" readonly>
							</div>
							<div class="form-group">
								<label>Email</label>
								<input type="email" class="form-control" name="email" value="<?= $result_v_info_akun['email']; ?>" readonly>
							</div>
							<?php } ?>
							<div class="form-group">
								<button class="btn btn-info" data-toggle="modal" data-target="#editInfoAkunModal" name="ubah_info_akun">Edit</button>
							</div>
						</div>
						<div class="col-lg-6">
							<h3>Ganti Password</h3>
							<form method="post" action="act_edit_akun.php" enctype="multipart/form-data">
								<div class="form-group">
								...
								<div class="form-group">
									...
									<input type="password" class="form-control" name="konf_password_baru" required>
								</div>
								<div class="form-group">
									<input type="submit" class="btn btn-primary" name="ganti_password" value="Change">
								</div>
							</form>
						</div>
					</div>
				...
				<div class="tab-pane fade" id="alamat" role="tabpanel" aria-labelledby="alamat-tab">
					<div class="row">
						...
						<div class="col-lg-6">
							<button class="btn btn-warning text-light" data-toggle="modal" data-target="#addAlamatModal" style="float: right;">Tambah Alamat</button>
						</div>
					</div>
					<br><br><br>
					<div class="row">
						...
						$no=1;
						$sql_alamat = mysqli_query($koneksi, "SELECT * FROM alamat_pelanggan INNER JOIN pelanggan ON pelanggan.id_pelanggan=alamat_pelanggan.id_pelanggan WHERE id_akun_pelanggan=$_SESSION[id_akun_pelanggan]");
						while($data_alamat = mysqli_fetch_array($sql_alamat)){
							if($data_alamat['ket'] == "1"){
								$ket = "btn btn-primary";
							} elseif($data_alamat['ket'] == "0"){
								$ket = "btn btn-outline-primary";
							}
						?>
						<div class="col-lg-12">
							<center>
								<a href="detail_alamat.php?id=<?php echo $data_alamat['id_alamat_pelanggan']; ?>&&page=<?= $page; ?>" class="<?= $ket; ?>" style="width: 500px;">
									<center>
										<?= $data_alamat['nama_penerima'] ?><br> (<?= $data_alamat['no_penerima'] ?>)<hr>
										<?= $data_alamat['alamat_1'] ?>, <?= $data_alamat['alamat_2'] ?>, <br>
										 <?= $data_alamat['desa_kelurahan'] ?>, <?= $data_alamat['kecamatan'] ?>, <?= $data_alamat['kab_kota'] ?>,<br>
										  <?= $data_alamat['provinsi'] ?>,<br> <?= $data_alamat['kode_pos'] ?>
									</center>
								</a>
							</center>
							<br>
