...
<head>
    ...
    <title>View Detail Pelanggan</title>
    ...
    <!-- End of Topbar -->
                ...
                <div class="container-fluid">
                        ...
                        <div class="card shadow mb-4">
                                    ...
                                    <a href="../data_pelanggan.php" class="btn btn-warning btn-icon-split">
                                    ...
                                    <div class="card-body">
                                        <?php
                                            include "../../../../koneksi.php";
                                            $id = $_GET['id'];
                                            $query = mysqli_query($koneksi, "SELECT * FROM pelanggan INNER JOIN akun_pelanggan ON pelanggan.id_akun_pelanggan=akun_pelanggan.id_akun_pelanggan WHERE id_pelanggan = '$id'");
                                            while($data = mysqli_fetch_array($query)){
                                        ?>
                                        <div class="row">
                                            <div class="col-4">
                                                <img src="../../../../pelanggan/web/market_log/img/profile/<?php echo $data['foto_pelanggan']; ?>" width="350px" height="350px" style="object-fit: cover; border-radius: 25px;">
                                            </div>
                                            
                                        <div class="col-8" >
                                            <h2 class="text-center"><strong><?= $data['nama_lengkap']; ?></strong></h2>
                                            <hr>
                                            <div class="row">
                                                <div class="col-6">
                                        ...
                                        <?php
                                        $alamat = mysqli_query($koneksi, "SELECT * FROM alamat_pelanggan WHERE ket='1' AND id_pelanggan = '$id'");
                                        while($data_alamat = mysqli_fetch_array($alamat)){
                                        ?>
                                        <div class="row">
                                            <div class="col-12">
                                                <hr>
                                                <h5 class="text-center">Alamat Pelanggan</h5>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-6">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control" value="<?= $data_alamat['nama_penerima']?>" readonly>
                                                </div>
                                            </div>
                                            <div class="col-6">
                                                <div class="form-group">
                                                    <label>No. HP Penerima</label>
                                                    <input type="text" class="form-control" value="<?= $data_alamat['no_penerima']?>" readonly>
                                                </div>
                                            </div>
                                            <div class="col-12">
                                                <div class="form-group">
                                                    <label>Baris Alamat 1</label>
                                                    <input type="text" class="form-control" value="<?= $data_alamat['alamat_1']?>" readonly>
                                                </div>
                                            </div>
                                            <div class="col-12">
                                                <div class="form-group">
                                                    <label>Baris Alamat 2</label>
                                                    <input type="text" class="form-control" value="<?= $data_alamat['alamat_2']?>" readonly>
                                                </div>
                                            </div>
                                            <div class="col-4">
                                                <div class="form-group">
                                                    <label>Desa/Kelurahan</label>
                                                    <input type="text" class="form-control" value="<?= $data_alamat['desa_kelurahan']?>" readonly>
                                                </div>
                                            </div>
                                            <div class="col-4">
                                                <div class="form-group">
                                                    <label>Kecamatan</label>
                                                    <input type="text" class="form-control" value="<?= $data_alamat['kecamatan']?>" readonly>
                                                </div>
                                            </div>
                                            <div class="col-4">
                                                <div class="form-group">
                                                    <label>Kabupaten/Kota</label>
                                                    <input type="text" class="form-control" value="<?= $data_alamat['kab_kota']?>" readonly>
                                                </div>
                                            </div>
                                            <div class="col-6">
                                                <div class="form-group">
                                                    <label>Provinsi</label>
                                                    <input type="text" class="form-control" value="<?= $data_alamat['provinsi']?>" readonly>
                                                </div>
                                            </div>
                                            <div class="col-6">
                                                <div class="form-group">
                                                    <label>Kode Pos</label>
                                                    <input type="text" class="form-control" value="<?= $data_alamat['kode_pos']?>" readonly>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                                    ...
                                    <?php
                                        $no = 1;
                                        $data_status = mysqli_query($koneksi,"select * from pesanan INNER JOIN pelanggan ON pesanan.id_pelanggan=pelanggan.id_pelanggan where nama_lengkap='$data[nama_lengkap]'");
                                        while($d_status = mysqli_fetch_array($data_status)){
                                            if($d_status['status'] == "Dikemas"){
                                                $status = "<span class='badge badge-primary'>$d_status[status]</span>";
                                            } else if($d_status['status'] == "Dibatalkan"){
                                                $status = "<span class='badge badge-danger'>$d_status[status]</span>";
                                            } else if($d_status['status'] == "Belum Bayar"){
                                                $status = "<span class='badge badge-info'>$d_status[status]</span>";
                                            } else if($d_status['status'] == "Dikirim"){
                                                $status = "<span class='badge badge-warning'>$d_status[status]</span>";
                                            } else if($d_status['status'] == "Selesai"){
                                                $status = "<span class='badge badge-success'>$d_status[status]</span>";
                                            } else if($d_status['status'] == "Menunggu Konfirmasi"){
                                                $status = "<span class='badge badge-primary'>$d_status[status]</span>";
                                            } else if($d_status['status'] == "Pengembalian"){
                                                $status = "<span class='badge badge-secondary'>$d_status[status]</span>";
                                            }
			                         ?>
                                        <tr>
                                            <td><?php echo $no++; ?></td>
                                            <td>INV-AC-<?php echo $d_status['id_pesanan']; ?></td>
                                            <td><?= $d_status['tanggal_pesan']; ?></td>
                                            <td><?= $d_status['nama_lengkap']; ?></td>
                                            
                                            <td><?= $status ?></span></td>
                                            <td><?php echo "Rp. ".number_format($d_status['total'])." ,-" ?></td>
                                        </tr>
                                    <?php 
		                                }}
		                            ?>
