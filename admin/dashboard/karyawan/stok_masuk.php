...
<head>
    ...
    <title>Data Stok Masuk Produk</title>
...
<div id="content">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-9">
                            ...
                            <div class="card shadow mb-4">
                                ...
                                <a href="#tabelDataStokMasuk" class="d-block card-header py-3" data-toggle="collapse"
                                    role="button" aria-expanded="true" aria-controls="tabelDataStokMasuk">
                                    <h6 class="m-0 font-weight-bold text-dark">Tabel Stok Produk Masuk</h6>
                                </a>
                                <!-- Card Content - Collapse -->
                                <div class="collapse show" id="tabelDataStokMasuk">
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-hover" id="dataTable" cellspacing="0">
                                            ...
                                            <?php
                                            $no1 = 1;
                                            $query = mysqli_query($koneksi,"SELECT * FROM produk_masuk INNER JOIN produk ON produk_masuk.id_produk=produk.id_produk ORDER BY tgl_masuk DESC");                                                while($data2 = mysqli_fetch_array($query)){
                                            ?>
                                            <tbody>
                                                <tr>
                                                    <td><?= $no1++; ?></td>
                                                    ...
                                                    <td><?= $data2['jumlah']; ?></td>
                                                    <td><a href="actions/produk/act_del_stok_produk.php?id=<?php echo $data2['id_masuk']; ?>" class="btn btn-danger btn-circle btn-sm alert_notif" title="Delete"><i class="fas fa-trash"></i></a>
                                                    </td>
                                                </tr>
                                            <?php 
		                                        }
                        ...
                        <div class="col-lg-3">
                            ...
                            <div class="card shadow mb-4">
                                ...
                                <a href="#collapseCardExample" class="d-block card-header py-3" data-toggle="collapse"
                                    ...
                                    <h6 class="m-0 font-weight-bold text-dark">Form Tambah Stok Produk</h6>
                                </a>
                                <!-- Card Content - Collapse -->
                                <div class="collapse show" id="collapseCardExample">
                                    <div class="card-body">
                                        <form action="actions/produk/act_add_stok_produk.php" method="post" enctype="multipart/form-data">
                                            <div class="form-group">
                                                ...
                                                ?>
                                                <option value="<?=$data1['id_produk']?>"><?=$data1['nama_produk']?></option> 
                                                <?php
                                                    }
                                                ?>
                                                </select>
                                            ...
                                            <input type="submit" class="btn btn-primary" name="tambahStok" value="Simpan">
                                        </form>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if(@$_SESSION['berhasil_tambah_produk']){ ?>
        <script>
            swal.fire("Selamat!", "<?php echo $_SESSION['berhasil_tambah_produk']; ?>", "success");
        </script>
    <!-- jangan lupa untuk menambahkan unset agar sweet alert tidak muncul lagi saat di refresh -->
    <?php unset($_SESSION['berhasil_tambah_produk']); } ?>

    <?php if(@$_SESSION['berhasil_edit_produk']){ ?>
        <script>
            swal.fire("Selamat!", "<?php echo $_SESSION['berhasil_edit_produk']; ?>", "success");
        </script>
    <!-- jangan lupa untuk menambahkan unset agar sweet alert tidak muncul lagi saat di refresh -->
    <?php unset($_SESSION['berhasil_edit_produk']); } ?>

    <?php if(@$_SESSION['berhasil_hapus_produk']){ ?>
        <script>
            swal.fire("Selamat!", "<?php echo $_SESSION['berhasil_hapus_produk']; ?>", "success");
        </script>
    <!-- jangan lupa untuk menambahkan unset agar sweet alert tidak muncul lagi saat di refresh -->
    <?php unset($_SESSION['berhasil_hapus_produk']); } ?>

    <?php if(@$_SESSION['gagal_hapus_produk']){ ?>
        <script>
            swal.fire("Oh Tidak!", "<?php echo $_SESSION['gagal_hapus_produk']; ?>", "error");
        </script>
    <!-- jangan lupa untuk menambahkan unset agar sweet alert tidak muncul lagi saat di refresh -->
    <?php unset($_SESSION['gagal_hapus_produk']); } ?>

    <?php if(@$_SESSION['berhasil_tambah_stok']){ ?>
        <script>
            swal.fire("Selamat!", "<?php echo $_SESSION['berhasil_tambah_stok']; ?>", "success");
        </script>
    <!-- jangan lupa untuk menambahkan unset agar sweet alert tidak muncul lagi saat di refresh -->
    <?php unset($_SESSION['berhasil_tambah_stok']); } ?>

    <?php if(@$_SESSION['gagal_tambah_stok']){ ?>
        <script>
            swal.fire("Oh Tidak", "<?php echo $_SESSION['gagal_tambah_stok']; ?>", "error");
        </script>
    <!-- jangan lupa untuk menambahkan unset agar sweet alert tidak muncul lagi saat di refresh -->
    <?php unset($_SESSION['gagal_tambah_stok']); } ?>

    <?php if(@$_SESSION['berhasil_hapus_stok_produk']){ ?>
        <script>
            swal.fire("Selamat!", "<?php echo $_SESSION['berhasil_hapus_stok_produk']; ?>", "success");
        </script>
    <!-- jangan lupa untuk menambahkan unset agar sweet alert tidak muncul lagi saat di refresh -->
    <?php unset($_SESSION['berhasil_hapus_stok_produk']); } ?>

    <?php if(@$_SESSION['gagal_hapus_stok_produk']){ ?>
        <script>
            swal.fire("Oh Tidak!", "<?php echo $_SESSION['gagal_hapus_stok_produk']; ?>", "error");
        </script>
    <!-- jangan lupa untuk menambahkan unset agar sweet alert tidak muncul lagi saat di refresh -->
    <?php unset($_SESSION['gagal_hapus_stok_produk']); } ?>
