...
$toDate = $_POST['toDate'];
$v_data = mysqli_query($koneksi, "SELECT * FROM transaksi_toko WHERE tanggal_order BETWEEN '$fromDate' AND '$toDate'");
...
<?php 
$data = mysqli_query($koneksi, "SELECT * FROM transaksi_toko WHERE tanggal_order BETWEEN '$fromDate' AND '$toDate'");
$q=0;
while ($row = mysqli_fetch_array($data)) {
    $tanggal = date('Y-m-d', strtotime($row['tanggal_order']));
    $jam = date('H:i', strtotime($row['tanggal_order']));
    $sql_sum = mysqli_query($koneksi, "SELECT SUM(total) AS total_pemasukkan FROM transaksi_toko WHERE tanggal_order BETWEEN '$fromDate' AND '$toDate'");
    $total_pemasukkan = mysqli_fetch_array($sql_sum);
$q++;?> 
<tr>
<td>&nbsp;&nbsp;INV-AC-OFF-<?php echo $row['id_transaksi']; ?></td>
